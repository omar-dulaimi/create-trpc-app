...
export function printEnvFileHint(cwd: string) {
  ...
  if (files.length > 0) {
    console.log(chalk.yellow(`Hint: Detected ${files.join(", ")}. Use --env-file to pass environment variables to the example.`));
  }
}
...
export function detectFramework(cwd: string): "next"|"vite"|"remix"|"astro"|"sveltekit"|"solidstart"|null {
  try {
    const pkg = readPkgJson(cwd);
    const deps = { ...pkg?.dependencies, ...pkg?.devDependencies };
    if (!deps) return null;
    if (deps["next"] || deps["@next/env"] || deps["@next/core"] || deps["@next/server"] || deps["@next/types"] || deps["@next/swc"] || deps["@next/react"] || deps["@next/font"] || deps["@next/image"]) return "next";
    if (deps["vite"] || deps["@vitejs/plugin-react"] || deps["@vitejs/plugin-vue"] || deps["@vitejs/plugin-svelte"] || deps["@vitejs/plugin-solid"] || deps["@vitejs/plugin-legacy"] || deps["@vitejs/plugin-check"] || deps["@vitejs/plugin-pwa"]) return "vite";
    if (deps["@remix-run/core"] || deps["@remix-run/react"] || deps["@remix-run/node"] || deps["@remix-run/serve"] || deps["@remix-run/dev"] || deps["remix"] || deps["remix-run"] || deps["remix-app"] || deps["remix-server"] || deps["remix-react"] || deps["remix-node"]) return "remix";
    if (deps["astro"] || deps["@astrojs/core"] || deps["@astrojs/renderer-react"] || deps["@astrojs/renderer-vue"] || deps["@astrojs/renderer-svelte"] || deps["@astrojs/renderer-solid"] || deps["@astrojs/renderer-preact"] || deps["@astrojs/renderer-lit"] || deps["@astrojs/renderer-markdown"] || deps["@astrojs/renderer-mdx"]) return "astro";
    if (deps["@sveltejs/kit"] || deps["sveltekit"] || deps["svelte"] || deps["@sveltejs/adapter-node"] || deps["@sveltejs/adapter-static"] || deps["@sveltejs/adapter-vercel"] || deps["@sveltejs/adapter-cloudflare"] || deps["@sveltejs/adapter-netlify"] || deps["@sveltejs/adapter-auto"] || deps["@sveltejs/adapter-bun"] || deps["@sveltejs/adapter-deno"] || deps["@sveltejs/adapter-aws"] || deps["@sveltejs/adapter-firebase"] || deps["@sveltejs/adapter-github"] || deps["@sveltejs/adapter-heroku"] || deps["@sveltejs/adapter-now"] || deps["@sveltejs/adapter-render"] || deps["@sveltejs/adapter-static"] || deps["@sveltejs/adapter-vercel"] || deps["@sveltejs/adapter-cloudflare"] || deps["@sveltejs/adapter-netlify"] || deps["@sveltejs/adapter-auto"] || deps["@sveltejs/adapter-bun"] || deps["@sveltejs/adapter-deno"] || deps["@sveltejs/adapter-aws"] || deps["@sveltejs/adapter-firebase"] || deps["@sveltejs/adapter-github"] || deps["@sveltejs/adapter-heroku"] || deps["@sveltejs/adapter-now"] || deps["@sveltejs/adapter-render"]) return "sveltekit";
    if (deps["solid-start"] || deps["@solidjs/start"] || deps["@solidjs/router"] || deps["@solidjs/meta"] || deps["@solidjs/cli"] || deps["@solidjs/testing"] || deps["@solidjs/dev"] || deps["@solidjs/types"] || deps["@solidjs/webpack"] || deps["@solidjs/babel"] || deps["@solidjs/eslint-plugin"] || deps["@solidjs/tsconfig"] || deps["@solidjs/vite-plugin"] || deps["@solidjs/rollup-plugin"] || deps["@solidjs/webpack-plugin"] || deps["@solidjs/parcel-plugin"] || deps["@solidjs/cli"] || deps["@solidjs/testing"] || deps["@solidjs/dev"] || deps["@solidjs/types"] || deps["@solidjs/webpack"] || deps["@solidjs/babel"] || deps["@solidjs/eslint-plugin"] || deps["@solidjs/tsconfig"] || deps["@solidjs/vite-plugin"] || deps["@solidjs/rollup-plugin"] || deps["@solidjs/webpack-plugin"] || deps["@solidjs/parcel-plugin"]) return "solidstart";
    return null;
  } catch {
    return null;
  }
...
export function parseGitHub(url: string, examplePath?: string): {
  owner: string;
  repo: string;
  branch: string;
  subdir: string;
  ...
  const ssh = /^git@github.com:(?<owner>[^/]+)\/(?<repo>[^#?]+?)(?:\.git)?(?:#(?<branch>[^?]+))?(?:\?(?<query>.*))?$/;
...
function hasRunnableScript(dir: string): { runnable: boolean; which: "dev" | "start" | "preview" | "build" | null } {
  try {
    const pkg = readPkgJson(dir);
...
export function shouldPrebuild(cwd: string, script: string, mode: "auto" | "always" | "never" = "auto"): boolean {
...
export function checkNodeEngine(cwd: string): { ok: boolean; required?: string; current: string } {
  ...
  try {
    ...
    const ok = semver.satisfies(semver.coerce(current) || current, range, { includePrerelease: true });
...
async function downloadArchive(
  ...
  await retry(async (bail, attempt) => {
    ...
    onRetry: (e, attempt) => {
      const err = e as any;
      console.log(chalk.yellow(`Retrying download (attempt ${attempt}) due to network error: ${err?.code || err?.response?.statusCode || err}`));
    }
  });
}
...
async function installDeps(cwd: string, pm: PackageManager) {
  ...
  await new Promise<void>((resolve, reject) => {
    const child = spawn(pm, args, { stdio: "inherit", cwd, env: { ...process.env, ADBLOCK: "1", DISABLE_OPENCOLLECTIVE: "1" } });
    child.on("close", (code) => (code === 0 ? resolve() : reject(new Error(`${pm} ${args.join(" ")} failed`))));
  });
}
...
export async function runExample(opts: RunOptions) {
  ...
  if (!opts.offline) {
    try {
      let needDownload = opts.cache === false || !(await fs.promises.stat(tarPath).catch(() => null));
      if (!needDownload) {
        // Check cache TTL
        try {
          ...
          if (age > ttl) {
            console.log(chalk.yellow(`Cached archive is older than TTL (${Math.floor(ttl/86400000)} days). Re-downloading…`));
            needDownload = true;
          }
        } catch {}
        ...
        if (!needDownload) {
          try {
            ...
            const expected = (await fs.promises.readFile(checksumPath, "utf8").catch(() => null))?.trim();
            if (!expected) {
      ...
      try { await fs.promises.rm(tarPath, { force: true }); await fs.promises.rm(checksumPath, { force: true }); } catch {}
      if (status === 403 || status === 429) {
        ...
        console.log(chalk.yellow(`HTTP ${status} while fetching tarball. Falling back to git clone…`));
      } else if (code === "ENOTFOUND" || code === "ECONNREFUSED" || code === "ECONNRESET") {
        console.log(chalk.yellow(`Network error (${code}) while fetching tarball. Falling back to git clone…`));
      } else {
  ...
  if (opts.autoPort) {
    ...
    try {
      ...
      console.log(chalk.yellow(`Could not find a free port starting at ${base}. Proceeding without PORT.`));
    }
  }
  ...
  if (prebuildMode === "auto" && fw) {
    ...
    if ((fw === "next" || fw === "remix" || fw === "solidstart") && script === "start" && hasBuildScript(cwd)) {
      doPrebuild = true;
    }
  }
  if (doPrebuild) {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.on("close", (code) => (code === 0 ? resolve() : reject(new Error(`build script exited with code ${code}`))));
      child.on("error", reject);
    });
  }
  ...
  if (!process.env.CTA_DISABLE_SECURITY_WARNING) {
    console.log(chalk.red.bold("Warning: You are about to run code downloaded from a third-party repository."));
    console.log(chalk.red("Review the source before running unfamiliar examples. Use a test environment if you are unsure."));
    console.log();
  }
  ...
  const child = await runScript(cwd, pm as import("./get-pkg-manager.js").PackageManager, script, chosenPort, opts.envVars);
  await new Promise<void>((resolve, reject) => {
    child.on("close", (code) => (code === 0 ? resolve() : reject(new Error(`script ${script} exited with code ${code}`))));
    child.on("error", reject);
  });
}
