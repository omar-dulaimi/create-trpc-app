...
export async function downloadRepoWithFallback(root: string, info: RepoInfo): Promise<void> {
  ...
  try {
    ...
    const zipStream = got.stream(
      `https://api.github.com/repos/${info.username}/${info.name}/zipball/${info.branch}`
    )
    ...
    try {
      ...
      try {
        execSync(
          `git clone --depth=1 --branch ${info.branch} https://github.com/${info.username}/${info.name}.git ${root}`
        )
        return
      } catch (e3) {
