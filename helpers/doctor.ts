...
export async function runDoctor(): Promise<void> {
  ...
  console.log(`${chalk.cyan("network to api.github.com")}: ${net.ok ? "ok" : chalk.red("unreachable")}`);
  if (!net.ok && net.error) {
    console.log(chalk.yellow(`hint: check proxy settings or retry later. ${net.error}`));
  }

  // Summary & guidance
  const issues = [!corepack.ok, !npm.ok && !pnpm.ok && !yarn.ok, !git.ok, !net.ok].filter(Boolean).length;
  if (issues === 0) {
    console.log();
